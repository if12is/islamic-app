...
class _NotesPageState extends ConsumerState<NotesPage> {
  ...
  List<QuranBookmark> _visibleNotes(List<QuranBookmark> all) {
    ...
    final notes = all.where((bookmark) {
      ...
      return QuranLocalService.normalizeArabic(bookmark.note).contains(needle) ||
          QuranLocalService.normalizeArabic(
            bookmark.surahName,
          ).contains(needle) ||
          QuranLocalService.normalizeArabic(bookmark.preview).contains(needle);
    }).toList();
    ...
    final saved = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: TextField(
          ...
          maxLines: 5,
          maxLength: 500,
          decoration: InputDecoration(hintText: dialogContext.tr('note_hint')),
        ),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(context.tr('my_reflections')),
          ...
          actions: [
            IconButton(
              ...
              onPressed: () => setState(() {
                _order = _order == _NotesOrder.newest
                    ? _NotesOrder.mushaf
                    : _NotesOrder.newest;
              }),
            ),
            IconButton(
              ...
              onPressed: () =>
                  _export(_visibleNotes(bookmarks.value ?? const [])),
            ),
          ],
        ),
        body: bookmarks.when(
